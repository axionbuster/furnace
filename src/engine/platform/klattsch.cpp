...
static float normalizedTilt(int tilt) {
  return 0.95f*(float)tilt/(float)((tilt<0)?128:127);
}
...
void DivPlatformKlattsch::loadInstrumentDefaults(Channel& ch, const DivInstrument& ins) {
  // Standard instruments were accepted before Klattsch had a dedicated type.
  // Keep their original instant, bank-derived profile for song compatibility.
  const bool legacy=ins.type!=DIV_INS_KLATTSCH;
  const DivInstrumentKlattsch& profile=ins.klattsch;
  ch.insTransitionTicks=legacy?0:profile.transition;
  ch.insVoicing=(legacy || profile.voicing==0xff)?-1.0f:(float)profile.voicing/255.0f;
  ch.insAspiration=(legacy || profile.aspiration==0xff)?-1.0f:(float)profile.aspiration/255.0f;
  if (legacy || profile.tilt==0xff) {
    ch.insTilt=-1.0f;
  } else {
    const int tilt=(profile.tilt<0x80)?profile.tilt:(profile.tilt-0x100);
    ch.insTilt=normalizedTilt(tilt);
  }
  ch.insEffort=(legacy || profile.effort==0xff)?-1.0f:(float)profile.effort/255.0f;
  ch.insVibRate=legacy?5.0f:(float)((profile.vibrato>>4)&15);
  ch.insVibDepth=legacy?0.0f:(float)(profile.vibrato&15)*4.0f;
  ch.insTremRate=legacy?5.0f:(float)((profile.tremolo>>4)&15);
  ch.insTremDepth=legacy?0.0f:(float)(profile.tremolo&15)/15.0f;
  ch.insGain=(legacy || profile.gain==0)?-1.0f:(float)profile.gain/16.0f;
  ch.insBwScale=(legacy || profile.bandwidth==0)?1.0f:(float)profile.bandwidth/64.0f;
  ch.insFormantShift=(legacy || profile.formantShift==0)?1.0f:(float)profile.formantShift/64.0f;

  if (!ch.transitionOverridden) ch.transitionTicks=ch.insTransitionTicks;
  if (!ch.voicingOverridden) ch.ovVoicing=ch.insVoicing;
  if (!ch.aspirationOverridden) ch.ovAspiration=ch.insAspiration;
  if (!ch.tiltOverridden) ch.ovTilt=ch.insTilt;
  if (!ch.effortOverridden) ch.ovEffort=ch.insEffort;
  if (!ch.vibratoOverridden) {
    ch.vibRate=ch.insVibRate;
    ch.vibDepth=ch.insVibDepth;
  }
  if (!ch.tremoloOverridden) {
    ch.tremRate=ch.insTremRate;
    ch.tremDepth=ch.insTremDepth;
  }
  if (!ch.gainOverridden) ch.ovGain=ch.insGain;
  if (!ch.bwScaleOverridden) ch.bwScale=ch.insBwScale;
  if (!ch.formantShiftOverridden) ch.formantShift=ch.insFormantShift;
}
...
void DivPlatformKlattsch::setFormantShift(Channel& ch, float shift) {
  const float oldShift=ch.formantShift;
  ch.formantShift=shift;
  if (!ch.synth || !ch.noteActive) return;

  klattsch::ParamId ids[6];
  size_t count=0;
  if (!ch.formantOverridden[0]) ids[count++]=klattsch::ParamId::F1;
  if (!ch.formantOverridden[1]) ids[count++]=klattsch::ParamId::F2;
  if (!ch.formantOverridden[2]) ids[count++]=klattsch::ParamId::F3;
  ids[count++]=klattsch::ParamId::BW1;
  ids[count++]=klattsch::ParamId::BW2;
  ids[count++]=klattsch::ParamId::BW3;
  scaleTargets(ch,ids,count,ch.formantShift/oldShift);
}
...
void DivPlatformKlattsch::tick(bool sysTick) {
  for (int i=0; i<chans; i++) {
    ...
    if (chan[i].std.ex1.had) {
      chan[i].ovVoicing=(float)CLAMP(chan[i].std.ex1.val,0,255)/255.0f;
      pushVoiceParam(chan[i],klattsch::ParamId::Voicing,chan[i].ovVoicing);
    }
    if (chan[i].std.ex2.had) {
      chan[i].ovAspiration=(float)CLAMP(chan[i].std.ex2.val,0,255)/255.0f;
      pushVoiceParam(chan[i],klattsch::ParamId::Aspiration,chan[i].ovAspiration);
    }
    if (chan[i].std.ex3.had) {
      const int tilt=CLAMP(chan[i].std.ex3.val,-128,127);
      chan[i].ovTilt=normalizedTilt(tilt);
      pushVoiceParam(chan[i],klattsch::ParamId::Tilt,chan[i].ovTilt);
    }
    if (chan[i].std.ex4.had) {
      chan[i].ovEffort=(float)CLAMP(chan[i].std.ex4.val,0,255)/255.0f;
      pushVoiceParam(chan[i],klattsch::ParamId::Effort,chan[i].ovEffort);
    }
...
int DivPlatformKlattsch::dispatch(DivCommand c) {
  switch (c.cmd) {
    case DIV_CMD_NOTE_ON:
      ...
      {
        DivInstrument* ins=parent->getIns(chan[c.chan].ins,DIV_INS_KLATTSCH);
        loadInstrumentDefaults(chan[c.chan],*ins);
    ...
    case DIV_CMD_PRE_PORTA:
      if (chan[c.chan].active && c.value2) {
        if (parent->song.compatFlags.resetMacroOnPorta) chan[c.chan].macroInit(parent->getIns(chan[c.chan].ins,DIV_INS_KLATTSCH));
      }
    ...
    case DIV_CMD_KLATTSCH_VOICING:
      chan[c.chan].voicingOverridden=c.value!=0xff;
      chan[c.chan].ovVoicing=(c.value==0xff)?chan[c.chan].insVoicing:((float)c.value/255.0f);
      pushVoiceParam(chan[c.chan],klattsch::ParamId::Voicing,chan[c.chan].ovVoicing);
      break;
    case DIV_CMD_KLATTSCH_ASPIRATION:
      chan[c.chan].aspirationOverridden=c.value!=0xff;
      chan[c.chan].ovAspiration=(c.value==0xff)?chan[c.chan].insAspiration:((float)c.value/255.0f);
    ...
    case DIV_CMD_KLATTSCH_TILT:
      if ((c.value&0xff)==0xff) {
        chan[c.chan].tiltOverridden=false;
        chan[c.chan].ovTilt=chan[c.chan].insTilt;
      } else {
        chan[c.chan].tiltOverridden=true;
        const int byteValue=c.value&0xff;
        const int signedValue=(byteValue<0x80)?byteValue:(byteValue-0x100);
        chan[c.chan].ovTilt=normalizedTilt(signedValue);
      }
      pushVoiceParam(chan[c.chan],klattsch::ParamId::Tilt,chan[c.chan].ovTilt);
      break;
    case DIV_CMD_KLATTSCH_EFFORT:
      chan[c.chan].effortOverridden=c.value!=0xff;
      chan[c.chan].ovEffort=(c.value==0xff)?chan[c.chan].insEffort:((float)c.value/255.0f);
      pushVoiceParam(chan[c.chan],klattsch::ParamId::Effort,chan[c.chan].ovEffort);
      break;
    case DIV_CMD_KLATTSCH_VIBRATO: {
      chan[c.chan].vibratoOverridden=true;
    ...
    case DIV_CMD_KLATTSCH_TREMOLO: {
      chan[c.chan].tremoloOverridden=true;
    ...
    case DIV_CMD_KLATTSCH_GAIN:
      chan[c.chan].gainOverridden=c.value!=0;
      chan[c.chan].ovGain=(c.value==0)?chan[c.chan].insGain:((float)c.value/16.0f);
    ...
    case DIV_CMD_KLATTSCH_BW_SCALE: {
      ...
      chan[c.chan].bwScaleOverridden=c.value!=0;
      chan[c.chan].bwScale=(c.value==0)?chan[c.chan].insBwScale:((float)c.value/64.0f);
    ...
    case DIV_CMD_KLATTSCH_FORMANT_SHIFT: {
      chan[c.chan].formantShiftOverridden=c.value!=0;
      setFormantShift(chan[c.chan],(c.value==0)?chan[c.chan].insFormantShift:((float)c.value/64.0f));
      break;
    }
    default:
      break;
