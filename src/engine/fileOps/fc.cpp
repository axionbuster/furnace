      ...
      for (int j=0; j<4; j++) {
        ...
        for (int k=0; k<32; k++) {
          ...
          if (fp.note[k]>0 && fp.note[k]<0x49) {
            ...
            p->newData[k][DIV_PAT_NOTE]=fp.note[k]+seq[i].transpose[j]+84+DIV_EDO31_LEGACY_OFFSET;
            ...
            p->newData[k][DIV_PAT_NOTE]=lastNote[j]+seq[i].transpose[j]+84+DIV_EDO31_LEGACY_OFFSET;
            // wrap-around if the note is too high
